...
export class GitPushViewProvider implements WebviewViewProvider {
  ...
  public openTab(context: ExtensionContext) {
    const panel = window.createWebviewPanel("git-push-tab", "Git Push", ViewColumn.Active, {
      enableScripts: true,
      localResourceRoots: [Uri.joinPath(context.extensionUri, "out")],
      retainContextWhenHidden: true,
    });
    GitPushViewProvider.activePanel = panel;
    const provider = new GitPushViewProvider(context);
    panel.webview.html = provider._getWebviewContent(panel.webview, context.extensionUri);
    const listener = new GitPushViewEventListener();
